...
- **CSV取り込み**: PayPay・クレジットカードの利用履歴を読み込み、登録前に明細の検索・編集・削除、チェック選択した明細の一括削除、重複確認ができます。UTF-8／Shift_JIS／UTF-16に対応し、見出しのない三井住友カード（Vpass）CSVも自動判定します。その他のカード会社独自CSVは列の割り当てを調整できます。
